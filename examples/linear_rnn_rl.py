...
from src.control_systems_torch import (DiGym, Masker, RnnModel, Gramians,
                                       get_device, StatefulGym)
from src.plotting import plot_phase_diagram
from src.ppo_recurrent import RecurrentPPO, ControlledActorMlpRnnPolicy
...
class EvaluationCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        episode_reward /= self.reward_norm
        train_accuracy = self.model.ep_info_buffer[-1]['r'] / self.reward_norm
        ...
        mlflow.log_metric(key='test_episode_length', value=episode_length,
                          step=n)
        mlflow.log_metric(key='training_reward', value=train_accuracy, step=n)
        plt.close()
        return True
...
def run_single(env: StatefulGym,
...
class LinearRlPipeline(NeuralPerturbationPipeline):
    def __init__(self, config: CfgNode):
        ...
        self.evaluation_callback = EvaluationCallback(config.training.NUM_TEST,
                                                      self.evaluate)
    ...
    def get_model(self, freeze_neuralsystem, freeze_actor, freeze_controller,
        ...
        model = RecurrentPPO(
            ControlledActorMlpRnnPolicy, environment, verbose=0,
    ...
    def train(self, perturbation_type, perturbation_level, dropout_probability,
        ...
        controlled_neuralsystem.assert_plasticity(
            freeze_neuralsystem=freeze_neuralsystem,
